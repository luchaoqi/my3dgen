...
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), required=False)
...
@click.option('--blur_fade_kimg', help='Blur over how many', metavar='INT',  type=click.IntRange(min=1), required=False, default=400)
...
@click.option('--density_reg_p_dist',    help='density regularization strength.', metavar='FLOAT', type=click.FloatRange(min=0), default=0.001, required=False, show_default=True)
...
# @click.option('--lora', help='Use LoRA', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--sym_regularization', help='Use symmetry regularization', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--lora', help='LoRA rank', metavar='INT',  type=click.IntRange(min=-1), default=-1, required=False, show_default=True)
@click.option('--id_regularization', help='Use identity regularization', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--adalora', help='Use AdaLoRA', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--lora_alpha', help='LoRA alpha', metavar='INT', type=click.IntRange(min=1), default=16, required=False, show_default=True)
@click.option('--freeze_render', help='Freeze rendering', metavar='BOOL',  type=bool, required=False, default=False)
...
def main(**kwargs):
    ...
    if c.training_set_kwargs.resolution == 512:
        ...
        if opts.cfg == 'afhq':
            # * 8XDC has tensor shape dismatch when resuming from afhqcats512-128.pkl
    ...
    c.sym_regularization = opts.sym_regularization
    c.lora = opts.lora
    c.id_regularization = opts.id_regularization
    c.adalora = opts.adalora
    if c.lora <= 0:
        c.adalora = False
    c.lora_alpha = opts.lora_alpha

    # Description string.
    # desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}'
    # desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}'
    desc = f'{opts.cfg:s}-{dataset_name:s}'
