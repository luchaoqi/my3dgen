...
class RankAllocator(object):
    ...
    def schedule_threshold(self, step:int):
        ...
        if step <= initial_warmup: 
            ...
            # todo: consider the case when curr_rank > self.total_rank
    ...
    def mask_to_target_rank(self, model, curr_rank): 
        ...
        with torch.no_grad():
            ...
            for n,p in model.named_parameters():
                if "lora_E" in n:
                    ...
                    if self.tb_writter is not None and self.global_step%self.log_interval==0:
                        self.tb_writter.add_scalar("Ranknum/%s"%(n,), ranknum, self.global_step // 1000) 
            ...
            if self.tb_writter is not None and self.global_step%self.log_interval==0:
                self.tb_writter.add_scalar("Budget/total_rank", curr_sum_rank, self.global_step // 1000)
                self.tb_writter.add_scalar("Budget/mask_threshold", mask_threshold, self.global_step // 1000)
                self.tb_writter.add_scalar("Budget/sum_param", sum_param, self.global_step // 1000)
                ...
                for n,p in model.named_parameters():
                    if "lora_A" in n or "lora_B" in n:
                        ...
                        self.tb_writter.add_scalar(
                            "Orth_regu_loss/%s"%n, orth_regu.item(), self.global_step // 1000
                        )
                self.tb_writter.add_scalar(
                    "train/orth_regu_loss", sum(regu_loss)/len(regu_loss), self.global_step // 1000
                )
